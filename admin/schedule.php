...
<body>
    ...
    <div class="dash-body">
        <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
            ...
            if ($_POST) {
                ...
                $sqlmain = "SELECT schedule.scheduleid, schedule.title, doctor.docname, schedule.scheduledate, schedule.nop,
                GROUP_CONCAT(CONCAT(schedule_slots.starttime, ' - ', schedule_slots.endtime) SEPARATOR ', ') as slots
                FROM schedule
                INNER JOIN doctor ON schedule.docid = doctor.docid
                LEFT JOIN schedule_slots ON schedule.scheduleid = schedule_slots.scheduleid";
                ...
                <td colspan="4">
                    <center>
                        <div class="abc scroll">
                            <table width="93%" class="sub-table scrolldown" border="1">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    if (!$result) {
                                        ...
                                        error_log("Database query error: " . $database->error);
                                        // You may want to exit or return here depending on your error handling strategy
                                    }
                                    ...
                                    <br><br><br><br>
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            $slots_result = $database->query("SELECT starttime, endtime FROM schedule_slots WHERE scheduleid = $scheduleid");
                                            ...
                                            if ($slots_result->num_rows > 0) {
                                                while ($slot_row = $slots_result->fetch_assoc()) {
                                                    ...
                                                    $slots .= '<a href="#" class="non-style-link"><button class="btn-primary-soft btn button-icon" style="padding-left: 40px; padding-top: 12px; padding-bottom: 12px; margin-top: 10px;"><font class="tn-in-text">' . substr($starttime, 0, 5) . ' - ' . substr($endtime, 0, 5) . '</font></button></a>&nbsp;&nbsp;&nbsp;';
                                                }
                                            }
                                            ...
                                            echo '<tr>
                                                    ...
                                                    <td style="text-align:start">' . $slots . '</td>
                                                    <td>
    ...
    if ($_GET) {
        ...
        if ($action == 'add-session') {
            ...
            // Loop through results to populate options
                                    ...
                                    <tr>
                                        <td class="label-td" colspan="2">
                                            <div id="slot-container">
                                                <div class="slot" style="display: flex; align-items: center; gap: 10px;">
                                                    <label for="slot_start" class="form-label">Start: </label>
                                                    <input type="time" name="slot_start[]" class="input-text slot-start" required>
                                                    <label for="slot_end" class="form-label">End: </label>
                                                    <input type="time" name="slot_end[]" class="input-text slot-end" required>
                                                </div>
                                            </div>
                                            <button type="button" onclick="addSlot()" class="login-btn btn-primary btn add-slot-button">Add Next Slot</button><br>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="2">
    ...
    <script>
        function addSlot() {
            ...
            const lastStart = lastSlot.querySelector('.slot-start').value;
            const lastEnd = lastSlot.querySelector('.slot-end').value;

            if (!lastStart || !lastEnd) {
                showValidationMessage('Please fill out the previous slot\'s start and end times before adding a new slot.');
                return;
            }

            if (lastStart >= lastEnd) {
                showValidationMessage('End time must be greater than start time for the previous slot.');
                return;
            }

            // Check if the new slot overlaps with any previous slot
            for (let i = 0; i < slots.length - 1; i++) {
                const startInput = slots[i].querySelector('.slot-start');
                const endInput = slots[i].querySelector('.slot-end');
                const start = startInput.value;
                const end = endInput.value;

                if ((lastStart >= start && lastStart < end) || (lastEnd > start && lastEnd <= end)) {
                    showValidationMessage('New slot overlaps with an existing slot.');
                    return;
                }
            }
    ...
    `;
        ...
        function showValidationMessage(message) {
            ...
            const notificationMessage = document.getElementById('validationMessage');
            const close = document.getElementsByClassName('close-notification')[0];
